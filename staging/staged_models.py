...
class GlobalStatistics(BaseModel):
    ...
    """
    kafkad_heartbeat: Optional[float] = 0.0  # Timestamp when kafkad.py last updated its status table
    kafka_alive: bool = False                # Is the remote kafka daemon alive?
    last_message: Optional[float] = 0.0      # Timestamp when the last valid file status message was received
    ...
    availdatasz: Optional[int] = 0           # scoutfs available data capacity
    health: Optional[int] = 0                # scoutfs health of filesystem: 0-unknown, 1-ok, 2-warning, 3-error
    healthstatus: Optional[str] = ''         # scoutfs health status info string
    releaseable: Optional[int] = 0           # scoutfs capacity of online files eligible for release (archive complete)
    totdatasz: Optional[int] = 0             # scoutfs total data capacity
    nonreleaseable: Optional[int] = 0        # scoutfs non-releaseable data size
    pending: Optional[int] = 0               # scoutfs size of data pending archiving
